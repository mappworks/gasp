...
public class Config implements ApplicationContextAware {
    ...
    public Config() {
        configFile = new ConfigFile(Paths.get(System.getProperty("user.home"), ".jasp", "config").toFile());
    }
     ...
     * <ol>
     *   <li>Look for a value by delegating to instances of {@link jasp.core.Config.ConfigDelegate} in the
     *   app context.</li>
     *   <li>Look for a servlet context parameter</li>
     *   <li>Look for a java system property</li>
     *   <li>Look for an environment variable</li>
     *   <li>Look for a value in ~/.jasp/config ini file</li>
     * </ol>
     * </p>
     * <p>
     ...
     *  <ol>
     *   <li>
     *       Servlet context parameters: <tt>(group, key) -> upper(JASP_&lt;group>_&lt;key>)</tt>. For example:
     *       <tt>(database, host) -> JASP_DATABASE_HOST</tt>
     *   </li>
     *   <li>
     *       Java system properties: <tt>(group, key) -> -Djasp.&lt;group>.&lt;key></tt>. For example:
     *       <tt>(database, host) -> -Djasp.database.host</tt>
     *   </li>
     *   <li>
    ...
    public Optional<String> get(String group, String key) {
        ...
        if (!val.isPresent() && appContext instanceof WebApplicationContext) {
            ...
            val = Optional.ofNullable(servletContext.getInitParameter(format("JASP_%S_%S", group, key)));
        }

        // next look for system property
        if (!val.isPresent()) {
            val = Optional.ofNullable(System.getProperty(format("jasp.%s.%s", group, key)));
        }

        // next look for environment variable
        if (!val.isPresent()) {
            val = Optional.ofNullable(System.getenv(format("JASP_%S_%S", group, key)));
        }

        // finally check ~/.jasp/config
        if (!val.isPresent()) {
