...
public class BaseCtrl {
    ...
    Optional<MediaType> responseFormat(HttpServletRequest req) {
        // first check file extension mapping
        Optional<MediaType> type = Optional.ofNullable(
            MAPPINGS.get(Files.getFileExtension(req.getRequestURI().toLowerCase())));

        return type.isPresent() ? type : parseAcceptHeader(req);
    }

    Optional<MediaType> parseAcceptHeader(HttpServletRequest req) {
